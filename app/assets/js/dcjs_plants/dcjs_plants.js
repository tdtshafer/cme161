...
d3.json("https://gentle-fjord-14489.herokuapp.com/iris", function(remote_json){
...
var sw_x = d3.extent(remote_json, function(d) { return d.sepal_width; }),
   pl_x = d3.extent(remote_json, function(d) { return d.petal_length; }),
  pw_x = d3.extent(remote_json, function(d) { return d.petal_width; }),
  sl_x = d3.extent(remote_json, function(d) { return d.sepal_length; });
  ...
  var sepal_length_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([sl_x[0]-1,sl_x[1]+1]) )
  ...
  var species_chart = dc
   ...
   var sepal_width_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([sw_x[0]-1,sw_x[1]+1]) )
    .xUnits(dc.units.fp.precision(.5));
   ...
   var petal_length_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([pl_x[0]-1,pl_x[1]+1]) )
    .xUnits(dc.units.fp.precision(.5));
   ...
   var petal_width_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([pw_x[0]-1,pw_x[1]+1]) )
    .xUnits(dc.units.fp.precision(.5));
...
var showButton = function(){
	if(petal_width_chart.filters().length > 0 || petal_length_chart.filters().length > 0 || sepal_width_chart.filters().length > 0 || sepal_length_chart.filters().length > 0 || 
